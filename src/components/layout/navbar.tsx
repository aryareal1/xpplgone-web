...
export default function NavBar() {
  ...
  return (
    <nav className="sticky top-0 z-50 h-15 w-full border-b bg-gray-50/80 backdrop-blur-sm dark:bg-gray-950/70">
      <div className="mx-auto flex h-full max-w-360 items-center justify-between px-4">
        <m.div
        ...
        >
          {navigations.map((v) => (
            <m.div variants={item} key={v.path}>
              <Button
                variant="ghost"
                size="sm"
                className={cn(
          ...
          <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent className="p-0 w-fit" align="end">
              {user ? (
                <Profile user={user} />
              ) : (
